...
function normalizeAlert(value) {
  ...
  return { mode: MODES.includes(value?.mode) ? value.mode : 'off', intervalSeconds: Number.isInteger(seconds) && seconds >= 10 && seconds <= 86400 ? seconds : 60 };
}
function normalizePreferences(value) {
  ...
  return { group: normalizeAlert(value?.group), private: normalizeAlert(value?.private), background: validColor || validImage ? background : { kind: 'color', value: DEFAULT_BACKGROUND } };
}
